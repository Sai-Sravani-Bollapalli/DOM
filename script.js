...
body.setAttribute("id","listingTable")
...
div1.setAttribute("class", "pageLinklist")

let prev=document.createElement("a");
prev.setAttribute("href","javascript:prevPage()");
prev.setAttribute("id","btn_prev")
prev.innerHTML="Previous";

let pages=document.createElement("ul");
pages.setAttribute("class","nav");
pages.setAttribute("id","pageNumberList")

let next=document.createElement("a");
next.setAttribute("href","javascript:nextPage()");
next.setAttribute("id","btn_next")
next.innerHTML="Next";

div1.append(prev,pages,next)
...
function numPages() {
    return Math.ceil(tableData.length / recordsPerPage);
}
...
function changePage(pagePosition) {
    ...
    if (pagePosition < 1) pagePosition = 1;
    if (pagePosition > numPages()) pagePosition = numPages();


    listing_table.innerHTML = "";
    for (var i = (pagePosition - 1) * recordsPerPage; i < (pagePosition * recordsPerPage); i++) {
        let output = `
    <tr>
    ...
    page_span.innerHTML = pagePosition;
}
changePage(1);
let pagerHtml;
let pageNumberList = document.getElementById('pageNumberList');
for (let page = 1; page <= 10; page++) {
    pagerHtml = `<li><span class="navItem" onclick='changePage(${page})'>${page}</span></li>`;
    pageNumberList.innerHTML += pagerHtml;
}